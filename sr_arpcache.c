...
void sr_arpcache_sweepreqs(struct sr_instance *sr) { 
	...
	if (difftime(now,iter->sent) > 1.0){ /*compare time*/
	if (iter->times_sent >= 5)
	{
		struct sr_packet* pkt_iter = iter->packets;
		while (pkt_iter)
		{
			icmp_handler(sr,pkt_iter->buf,3,1);
			pkt_iter = pkt_iter->next;
		}
		sr_arpreq_destroy(&sr->cache,iter);

	}

	else 
	{
		forwardNewRequest(sr,iter);	
		iter->times_sent++; /*update # times sent as well as time of request sent*/
		iter->sent = now;
